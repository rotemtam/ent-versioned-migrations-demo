...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withBlogPosts; query != nil {
		if err := uq.loadBlogPosts(ctx, query, nodes,
			func(n *User) { n.Edges.BlogPosts = []*Blog{} },
			func(n *User, e *Blog) { n.Edges.BlogPosts = append(n.Edges.BlogPosts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadBlogPosts(ctx context.Context, query *BlogQuery, nodes []*User, init func(*User), assign func(*User, *Blog)) error {
	...
	for _, n := range neighbors {
		fk := n.user_blog_posts
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_blog_posts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_blog_posts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
